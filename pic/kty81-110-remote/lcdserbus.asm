...
;Unterprogramme für I2C zu 8-Bit-Betrieb zum LCD mit 2 Controllern über 2x PCF8574, 
; einfügen mit:
;	#include <lcdserbus.asm>
;
;setzt im Hauptprogramm voraus:
;	#include <I2C-PIC-Master.asm>	; Hardware-I2C für PIC im Master Mode
;	oder 
;	#include	<software-iic.asm>	; Software-I2C für PIC als Master
;	#include <quarz_4MHz.asm>
;	oder 
;	#include <quarz_2097152Hz.asm>	;Zeitverzöger.
...
;LcdStat	equ	0x32	; UP Busyabfrage, Inhalt des LCD-Statusregister
...
;
;Anschlußbelegung je nach LCD-Typ unterschiedlich,
; hier für MSP-C404DYSY-1N:
...
;#define	LcdBel	LcdByte,7	; Displaybeleuchtung am PCF P7  (  1=Aus, 0=Ein )
...
;
;Wertzuweisungen für Adressregister der PCF8574:
;	movlw	b'01000110'	; PCF-Adr. 3, lsb=0 (schreiben zum Slave)
;	movwf	PcfAdr1		; Adr.-Reg. für IC1
;	movlw	b'01001000'	; PCF-Adr. 4, lsb=0 (schreiben zum Slave)
;	movwf	PcfAdr2		; Adr.-Reg. für IC2
;
;##############################################################
; UP InitLcdSer: für LCD-Initialisierung, 3x Steuerbyte '00110000' senden.
; Bei LCD's mit 2 Controllern an LC1 und LC2 denken !
;##############################################################
InitLcdSer
...
BusyLoopS
	...
	; nun legt das Display den Inhalt seines Steuerregisters an P7:0 von IC1
	call	i2c_off
	...
	call	i2c_tx		; Adresse senden
	call	i2c_rx		; vom Slave lesen (Steuerregisterinhalt nach w)
...
; Übergabe des Datenbyte in w
; vor Aufruf muß in LcdCon,0 stehen, welcher Controller aktiv sein soll
;##############################################################
OutLcdDat
	movwf	LcdDaten	; Byte in LcdDaten zwischenspeichern
	call	LcdBusySer	; warten, bis Display bereit ist
...
;##############################################################
; UP LcdConEnOn: 
; wählt je nach Controller LcdE oder LcdE2 aus und setzt dieses Bit.
; Wurde die Auswahl (LC1=1 oder LC2=1) vergessen, 
; wird automatisch Controller 1 aktiviert.
;##############################################################
LcdConEnOn
	btfsc	LC1
	goto	control1on
...
;##############################################################
; UP LcdConEnOut: 
; wählt je nach Controller LcdE oder LcdE2 aus und löscht dieses Bit.
; Wurde die Auswahl (LC1=1 oder LC2=1) vergessen, 
; wird automatisch E/E1=0 (Controller 1 deaktiviert).
;##############################################################
LcdConEnOut
	btfsc	LC1
	goto	control1out
